...
export default function Home() {
    ...
    const saveMockup = (e) => {
        ...
        Object.entries(frameData).map(item => {
            formData.append(item[0], item[1])
        })
        Object.entries(overlayData).map(item => {
            formData.append(item[0], item[1])
        })
    ...
    if (session)
        if (localStorage.getItem('type') === 'admin')
            if (props.name)
                        ...
                        <div className="min-h-[90vh] bg-gray-100  grid grid-cols-4">
                            <div className="col-span-4 md:col-span-3 flex justify-center items-center overflow-hidden">
                                <div className="box relative" >
                                    <Image className={`mock relative w-full ${preview ? 'z-20' : 'z-0'}`} alt="" src={props.imgSrc} width={100} height={100} />
                                    <Rnd default={{
                                        x: 50,
                                        y: 50,
                                        width: 100,
                ...
                router.push('/admin/mockups/upload')
        else
            return (
